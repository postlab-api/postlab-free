...
export const authEvents$ = new Subject<TUserAuthEvent | { event: 'token_refresh' }>()
const currentUser$ = new BehaviorSubject<TUserInfo | null>(null)
export const probableUser$ = new BehaviorSubject<TUserInfo | null>(null)

const APP_DATA_PATH = '~/.hopp-desktop-app-data.dat'

const persistenceService = new Container().bind(PersistenceService)
...
async function getInitialUserDetails() {
  ...
  try {
    ...
    let res = await client.post(`${import.meta.env.VITE_BACKEND_GQL_URL}`, Body.json(body), {
      headers: {
...
function setUser(user: TUserInfo | null) {
...
export const def: TUserAuthPlatformDef = {
  ...
  onBackendGQLClientShouldReconnect(func) {
    authEvents$.subscribe((event) => {
      if (event.event == 'login' || event.event == 'logout' || event.event == 'token_refresh') {
        func()
      }
    })
  },
