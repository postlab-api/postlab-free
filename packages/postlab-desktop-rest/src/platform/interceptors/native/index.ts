import { CookieJarService } from '@postlab/services/cookie-jar.service'
import {
  Interceptor,
  InterceptorError,
  RequestRunResult,
} from '@postlab/services/interceptor.service'
...
import { PersistenceService } from '@postlab/services/persistence'
...
type FormDataEntry = {
  key: string
  value: TInterceptorsFormDataValue
}

type BodyDef = { Text: string } | { URLEncoded: KeyValuePair[] } | { FormData: FormDataEntry[] }
...
export const preProcessRequest = (req: AxiosRequestConfig): AxiosRequestConfig => {
...
async function convertToRequestDef(
  ...
  return {
    ...
    headers: Object.entries(axiosReq.headers ?? {})
      .filter(
        ([key, value]) =>
          !(key.toLowerCase() === 'content-type' && value.toLowerCase() === 'multipart/form-data'),
      ) // Removing header, because this header will be set by relay.
      .map(([key, value]): KeyValuePair => ({ key, value })),
    parameters: Object.entries((axiosReq.params as Record<string, string>) ?? {}).map(
...
export class NativeInterceptorService extends Service implements Interceptor {
  ...
  override onServiceInit() {
    ...
    const persistedProxyInfo = this.persistenceService.getLocalConfig(PROXY_INFO_PERSIST_KEY)
    ...
    if (caStoreDataParseResult.type === 'ok') {
      this.caCertificates.value = caStoreDataParseResult.value.certs.map((entry) => ({
        ...entry,
    ...
    watch(this.caCertificates, (certs) => {
      const storableValue: CACertStore = {
        v: 1,
        certs: certs.map((el) => ({ ...el, certificate: Array.from(el.certificate) })),
      }
    ...
    if (clientCertStoreDataParseResult.type === 'ok') {
      this.clientCertificates.value = new Map(
        Object.entries(clientCertStoreDataParseResult.value.clientCerts).map(([domain, cert]) => {
  ...
  public runRequest(req: AxiosRequestConfig): RequestRunResult<InterceptorError> {
    ...
    const relevantCookies = this.cookieJarService.getCookiesForURL(new URL(processedReq.url!))
