...
/// ```
#[tauri::command]
pub async fn interop_startup_init<R: Runtime>(window: WebviewWindow<R>) -> StartupResult<()> {
...
mod tests {
    ...
    async fn test_interop_startup_init_main_window_found_amongst_others() {
        ...
        let _ = WebviewWindowBuilder::new(&app, "other", WebviewUrl::default())
            .build()
            .expect("Failed to create other window");

        let window = app.get_webview_window("other").expect("`other` window not found");
